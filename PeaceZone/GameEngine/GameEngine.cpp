...
//Main Game Loop
std::string GameEngine::mainGameLoop(int turnLimit) {

	std::cout << "Starting Main Game Loop" << std::endl;
	int iterationCounter = turnLimit;
...
void GameEngine::chooseInputMode(std::string inputString) {
...
void Tournament::tournamentMode(std::string tournamentString) {
	...
	// Parse string into individual paraemeters
	std::string parameters = tournamentString.substr(tournamentString.find(" ") + 1, tournamentString.size());

	std::string map = parameters.substr(parameters.find("-M ") + 3, parameters.find("-P") - (parameters.find("-M ") + 3));
	std::string playerStrategies = parameters.substr(parameters.find("-P ") + 3, parameters.find("-G") - (parameters.find("-P ") + 3));
	std::string games = parameters.substr(parameters.find("-G ") + 3, parameters.find("-D") - (parameters.find("-G ") + 3));
	std::string turns = parameters.substr(parameters.find("-D ") + 3, parameters.size() - parameters.find("-D ") + 3);

	// Parse into list of map names
	...
	int mapNumber = mapList.size();

	// Parse into list of play strategies
	delimiter = ", ";
	pos = 0;
	token = "";
	...
	// Parse into number of turns
	turnLimit = stoi(turns);
	...
	for (int i = 0; i < mapNumber; i++)
	{
		...
		for (int j = 0; j < gamesNumber; j++)
		{
			...
			for (int k = 0; k < strategiesList.size(); k++)
			{
				...
				{
					gameEngine->playerList.at(k)->strategy = new AggressivePlayerStrategy();
				}
				else if (gameEngine->playerList.at(k)->name.find("Cheater") != std::string::npos)
				{
					gameEngine->playerList.at(k)->strategy = new CheaterPlayerStrategy();
				}
				else if (gameEngine->playerList.at(k)->name.find("Benevolent") != std::string::npos)
				{
					gameEngine->playerList.at(k)->strategy = new BenevolentPlayerStrategy();
				}
				else if (gameEngine->playerList.at(k)->name.find("Neutral") != std::string::npos)
				{
					gameEngine->playerList.at(k)->strategy = new NeutralPlayerStrategy();
				}
			}
	...
	// Write the round commands to the file
	output.open(reportFile, std::ios_base::out);
	output << reportString << std::endl;
	output.close();
}
